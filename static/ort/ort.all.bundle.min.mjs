...
`}return me};return`
  ...
  }`},Vb=(r,e,n,t,o,i,s,a)=>{let u=e.format==="NHWC",l=u?r[0].dims[3]:r[0].dims[1],f=n[0],c=u?n[2]:n[3],p=u?n[1]:n[2],b=u?n[3]:n[1],g=u&&l%4===0&&l%3&&b%4===0,h=u?b:c*p,T=u?c*p:b,v=[8,8,1],w=t<=8?[4,1,1]:[4,4,1],_=[Math.ceil(h/v[0]/w[0]),Math.ceil(T/v[1]/w[1]),Math.ceil(f/v[2]/w[2])];ve("verbose",()=>`[conv_backprop_mm_webgpu] dispatch = ${_}`);let $=g?4:1,P=Math.max(v[0]*$,v[1]),E=g?4:1,B=[e.kernelShape[u?1:2],e.kernelShape[u?2:3]],N=[B[0]+(e.dilations[0]<=1?0:(B[0]-1)*(e.dilations[0]-1)),B[1]+(e.dilations[1]<=1?0:(B[1]-1)*(e.dilations[1]-1))],H=[N[0]-1-Math.floor((e.pads[0]+e.pads[2])/2),N[1]-1-Math.floor((e.pads[1]+e.pads[3])/2)],j=[{type:6,data:t},{type:6,data:o},{type:6,data:i},{type:6,data:e.strides},{type:6,data:e.dilations},{type:6,data:B},{type:6,data:H}];Pt(e,j),j.push(...U(r[0].dims,r[1].dims));let V=["rank","rank"];s&&(j.push(...U(r[2].dims)),V.push("rank")),j.push(...U(n));let ae=be=>{let ye=L("x",r[0].dataType,r[0].dims.length,E),_e=L("w",r[1].dataType,r[1].dims.length,1),ie=F("result",r[0].dataType,n.length,E),Ae=[ye,_e],$e="";if(s){let me=L("bias",r[2].dataType,r[2].dims.length,E);Ae.push(me),$e+=`
