<script>
	...
	import { modelsStore, modelTypesStore, dataTypesStore } from '../store/store';
	...
	const toggleDataTypes = () => {
		...
		const allDataTypes = models
			.filter((model) => model.id === modelId)
			.map((model) => model.datatype);
	...
	beforeUpdate(() => {
		...
		for (let i = 0; i < allModelTypes.length; i++) {
			modelTypes[allModelTypes[i]] = false;
		}

		for (let i = 0; i < allDataTypes.length; i++) {
			dataTypes[allDataTypes[i]] = false;
		}
...
<div class="types">
	<div>
		{#if modelTypes && modelTypes.hasOwnProperty('onnx')}
		...
		{/if}
		{#if modelTypes && modelTypes.hasOwnProperty('tflite')}
		...
		{/if}
		{#if modelTypes && modelTypes.hasOwnProperty('npy')}
		...
		{/if}
		{#if modelTypes && modelTypes.hasOwnProperty('pt')}
