<script>
	...
	const fmt2 = (v) => (v === null ? 'N/A' : (typeof v === 'number' && isFinite(v) ? v.toFixed(2) : (v ?? '').toString()));
	const str = (v) => (v === null ? 'N/A' : (v ?? '').toString());
					...
					{#if key.wasm_1 && key.wasm_1.status !== 0}
						...
						{#if key.wasm_1.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.wasm_1.loadcompilation)} ms; Compilation Time: {str(key.wasm_1.compilation)} ms; First Inference Time: {key.wasm_1.warmup.toString()} ms; Time to First Inference: {key.wasm_1.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
									{#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.wasm_1.loadcompilation)}</span
										>{/if}
					...
					{#if key.wasm_4 && key.wasm_4.status !== 0}
						...
						{#if key.wasm_4.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.wasm_4.loadcompilation)} ms; Compilation Time: {str(key.wasm_4.compilation)} ms; First Inference Time: {key.wasm_4.warmup.toString()} ms; Time to First Inference: {key.wasm_4.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
									{#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.wasm_4.loadcompilation)}</span
										>{/if}
									{#if resultOptions.compilation}<span class="compilation"
					...
					{#if key.webnn_cpu && key.webnn_cpu.status !== 0}
						...
						{#if key.webnn_cpu.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.webnn_cpu.loadcompilation)} ms; Compilation Time: {str(key.webnn_cpu.compilation)} ms; First Inference Time: {key.webnn_cpu.warmup.toString()} ms; Time to First Inference: {key.webnn_cpu.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
									{#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.webnn_cpu.loadcompilation)}</span
										>{/if}	
									{#if resultOptions.compilation}<span class="compilation"
					...
					{#if key.webgl && key.webgl.status !== 0}
						...
						{#if key.webgl.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.webgl.loadcompilation)} ms; Compilation Time: {str(key.webgl.compilation)} ms; First Inference Time: {key.webgl.warmup.toString()} ms; Time to First Inference: {key.webgl.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
								  {#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.webgl.loadcompilation)}</span
										>{/if}
									{#if resultOptions.compilation}<span class="compilation"
					...
					{#if key.webgpu && key.webgpu.status !== 0}
						...
						{#if key.webgpu.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.webgpu.loadcompilation)} ms; Compilation Time: {str(key.webgpu.compilation)} ms; First Inference Time: {key.webgpu.warmup.toString()} ms; Time to First Inference: {key.webgpu.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
								  {#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.webgpu.loadcompilation)}</span
										>{/if}
									{#if resultOptions.compilation}<span class="compilation"
					...
					{#if key.webnn_gpu && key.webnn_gpu.status !== 0}
						...
						{#if key.webnn_gpu.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.webnn_gpu.loadcompilation)} ms; Compilation Time: {str(key.webnn_gpu.compilation)} ms; First Inference Time: {key.webnn_gpu.warmup.toString()} ms; Time to First Inference: {key.webnn_gpu.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
								  {#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.webnn_gpu.loadcompilation)}</span
										>{/if}
									{#if resultOptions.compilation}<span class="compilation"
					...
					{#if key.webnn_npu && key.webnn_npu.status !== 0}
						...
						{#if key.webnn_npu.status === 3}
							<div
								...
								title="LoadAndCompile Time: {str(key.webnn_npu.loadcompilation)} ms; Compilation Time: {str(key.webnn_npu.compilation)} ms; First Inference Time: {key.webnn_npu.warmup.toString()} ms; Time to First Inference: {key.webnn_npu.timetofirstinference.toString()} ms; Inference Time (Median): {key
							...
							>
								...
								>
								  {#if resultOptions.loadcompilation}<span class="loadcompilation"
											>{fmt2(key.webnn_npu.loadcompilation)}</span
										>{/if}
									{#if resultOptions.compilation}<span class="compilation"
