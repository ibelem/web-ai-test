...
<div id="result">
	...
	{#if testQueue.length === 0}
		{#if results.length > 0}
			...
			<div class="result">
				<div class="q title _{selectedBackends.length}">
					...
					{#if results[0].webnn_cpu_1 && results[0].webnn_cpu_1.status !== 0}<div
							class="backend cpu"
						>
							<span title="WebNN CPU with 1 Thread on CPU"
								><span class="hide">Web</span>NN CPU 1T
					...
					{#if results[0].webnn_cpu_4 && results[0].webnn_cpu_4.status !== 0}<div
							class="backend cpu"
						>
							<span title="WebNN CPU with 4 Threads on CPU"
								><span class="hide">Web</span>NN CPU 4T
			...
			<div class="q copy">
				<div>
					...
					{#if testQueue.length === 0}
						<button
							title="Download screenshot of test results"
		...
		{#if results.length > 0}
			<div class="rqtitle">
				<div class="title rq">First Inference Time</div>
			</div>
			<div class="result">
				<div class="q title _{selectedBackends.length}">
					<div class="m" title="Model Name">
					...
					{:else} -->
						...
						{#if key.webnn_cpu_1 && key.webnn_cpu_1.status !== 0}
							...
							{#if key.webnn_cpu_1.status === 3}
								...
								>
									<span>
										{key.webnn_cpu_1.warmup.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_cpu_4 && key.webnn_cpu_4.status !== 0}
							...
							{#if key.webnn_cpu_4.status === 3}
								...
								>
									<span>
										{key.webnn_cpu_4.warmup.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_gpu && key.webnn_gpu.status !== 0}
							...
							{#if key.webnn_gpu.status === 3}
								...
								>
									<span>
										{key.webnn_gpu.warmup.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_npu && key.webnn_npu.status !== 0}
							...
							{#if key.webnn_npu.status === 3}
								...
								>
									<span>
										{key.webnn_npu.warmup.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
		...
		{#if results.length > 0}
			<div class="rqtitle">
				<div class="title rq">Compilation Time</div>
			</div>
			<div class="result">
				<div class="q title _{selectedBackends.length}">
					<div class="m" title="Model Name">
					...
					{:else} -->
						...
						{#if key.wasm_1 && key.wasm_1.status !== 0}
							...
							{#if key.wasm_1.status === 3}
								...
								>
									<span>
										{key.wasm_1.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.wasm_4 && key.wasm_4.status !== 0}
							...
							{#if key.wasm_4.status === 3}
								...
								>
									<span>
										{key.wasm_4.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_cpu_1 && key.webnn_cpu_1.status !== 0}
							...
							{#if key.webnn_cpu_1.status === 3}
								...
								>
									<span>
										{key.webnn_cpu_1.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_cpu_4 && key.webnn_cpu_4.status !== 0}
							...
							{#if key.webnn_cpu_4.status === 3}
								...
								>
									<span>
										{key.webnn_cpu_4.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webgl && key.webgl.status !== 0}
							...
							{#if key.webgl.status === 3}
								...
								>
									<span>
										{key.webgl.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webgpu && key.webgpu.status !== 0}
							...
							{#if key.webgpu.status === 3}
								...
								>
									<span>
										{key.webgpu.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_gpu && key.webnn_gpu.status !== 0}
							...
							{#if key.webnn_gpu.status === 3}
								...
								>
									<span>
										{key.webnn_gpu.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
						...
						{#if key.webnn_npu && key.webnn_npu.status !== 0}
							...
							{#if key.webnn_npu.status === 3}
								...
								>
									<span>
										{key.webnn_npu.compilation.toFixed(2)}
									</span>
									<button
										on:click={() =>
											copyRawInference(
