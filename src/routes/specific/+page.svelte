...
<div>
	...
	<div class="title tq int8">Relaxed Simd Optimization Tests · 3</div>
	<div class="tq benchmark int8">
		{#each uniqueModels as model}
			{#if model !== 'model_access_check'}
				{#if getModelDataTypeById(model) === 'int8' 
					&& (model === 'moondream2_vision_encoder_int8'
						|| model === 'nomic_embed_text_v1_int8'
						|| model === 'florence2_decoder_int8'
						)
			  }
					<div
					...
					>
						...
						<a href="{base}/run/{model}?backend=wasm_1&run=100&modeltype=onnx&datatype=int8&relaxedsimd=0" class="titlemark"
					...
					>
						...
						<a href="{base}/run/{model}?backend=wasm_1&run=100&modeltype=onnx&datatype=int8&relaxedsimd=1" class="titlemark"
							>{@html getHTMLModelName(model)}
							{#if getModelSizeById(model)}<span>{getModelSizeById(model)}</span>{/if} <span class="relaxed_simd">Relaxed SIMD</span></a
						>
